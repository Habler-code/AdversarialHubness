...
<img src="docs/images/detection-metrics.png" alt="Detection Metrics" width="450"/>
...
<img src="docs/images/adversarial-hubness-concept.png" alt="Adversarial Hubness Concept" width="450"/>
...
- **Advanced Detection Modes**:
  - **Concept-Aware Detection**: Detects hubs that dominate within specific semantic categories but may be hidden in global statistics
    - Groups queries by concept (from metadata or automatic clustering)
    - Computes hub rate per concept
    - Flags documents with high z-scores in ANY concept
    - **Best for**: News/articles with categories, e-commerce with product types, knowledge bases with topics
    - See [Concept and Modality Guide](docs/CONCEPTS_AND_MODALITIES.md) for details
  - **Modality-Aware Detection**: Detects hubs exploiting cross-modal retrieval patterns
    - Tracks query and document modalities
    - Computes hits per modality combination
    - Flags documents with anomalous cross-modal retrieval
    - **Best for**: Image-text search systems, audio-text systems, any multi-modal RAG
    - See [Concept and Modality Guide](docs/CONCEPTS_AND_MODALITIES.md) for details
...
- **Multi-Index Late Fusion** (Gold Standard Multimodal Architecture):
  - Supports separate embedding spaces for each modality (text, image, etc.)
  - Parallel retrieval from modality-specific indexes
  - Late fusion using RRF, weighted sum, or max scoring
  - Diversity enforcement to prevent redundant results
  - **Best for**: Secure multimodal RAG systems
  - See [Concept and Modality Guide](docs/CONCEPTS_AND_MODALITIES.md) for architecture details
...
- **Flexible Input Modes** (Plug-and-Play Architecture):
  ...
  - `multi_index`: Multi-index mode for multimodal systems with separate indexes
...
<img src="docs/images/multi-ranking-pipeline.png" alt="Multi-Ranking Detection Pipeline" width="450"/>
...
<img src="docs/images/vector-db-abstraction.png" alt="Vector Database Abstraction" width="450"/>
...
- **Detectors**: Enable/configure detection algorithms (hubness, cluster spread, stability, dedup, concept-aware, modality-aware)
...
detectors:
  ...
  concept_aware:
    enabled: true
    mode: metadata  # or query_clustering, hybrid
...
For complete configuration reference, see [Usage Guide](docs/USAGE.md) and example configurations in `examples/configs/` and `benchmarks/configs/`.
...
- **Concept Hub Z**: Per-topic hubness z-score (concept-aware detection)
- **Cross-Modal Hub Z**: Cross-modal hubness z-score (modality-aware detection)
...
│   │   ├── concepts/   # Concept-aware detection
│   │   ├── modalities/ # Modality-aware detection
│   │   ├── fusion/     # Late fusion for multi-index
...
- **Benchmarks**: See [benchmarks documentation](benchmarks/README.md) for evaluation methodology and results
